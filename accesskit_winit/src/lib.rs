...
impl Adapter {
    ...
    /// Creates a new AccessKit adapter for a winit window without requiring
    /// an EventLoopWindowTarget. This works on platforms where the underlying
    /// adapter doesn't actually use the EventLoopWindowTarget parameter.
    ///
    /// # Panics
    ///
    /// Panics if the window is already visible or if called on a platform that
    /// requires the EventLoopWindowTarget.
    #[cfg(target_os = "macos")]
    pub fn with_event_loop_proxy_simple<T: From<Event> + Send + 'static>(
        window: &Window,
        proxy: EventLoopProxy<T>,
    ) -> Self {
        if window.is_visible() == Some(true) {
        ...
        let inner = platform_impl::Adapter::new_simple(
            window,
            activation_handler,
            action_handler,
            deactivation_handler,
